...
# RUN:     -riscv-no-aliases -show-encoding < %s 2>&1 \
# RUN:     | FileCheck -check-prefixes=CHECK-NO-EXT-FZCF %s
...
# CHECK-NO-EXT-FZCF:  error: instruction requires the following: 'C' (Compressed Instructions) or 'Zcf' (c.flw, c.flwsp, c.fsw, c.fswsp), 'F' (Single-Precision Floating-Point)
