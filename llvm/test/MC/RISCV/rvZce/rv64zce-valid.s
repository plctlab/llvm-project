...
# CHECK-NO-EXT: error: instruction requires the following: 'Zcee' (high performance cores v0.50)
c.zext.b s0
...
# CHECK-NO-RV64-AND-EXT: error: instruction requires the following: 'Zcee' (high performance cores v0.50), RV64I Base Instruction Set
c.zext.w s0

# CHECK-ASM-AND-OBJ: c.neg s0
# CHECK-ASM: encoding: [0x18,0x80]
# CHECK-NO-EXT: error: instruction requires the following: 'Zcea' (all existing standard extensions for small embedded cores v0.50)
c.neg s0
...
# CHECK-NO-EXT: error: instruction requires the following: 'Zceb' (reuses encoding from the D-extension, incompatible with D v0.50)
# CHECK-NO-RV64: error: instruction requires the following: RV64I Base Instruction Set
# CHECK-NO-RV64-AND-EXT: error: instruction requires the following: 'Zceb' (reuses encoding from the D-extension, incompatible with D v0.50), RV64I Base Instruction Set
ldgp s0, 65528(gp)
