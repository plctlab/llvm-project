...
public:
  ...
  bool isNZUImm5() const {
    int64_t Imm;
    RISCVMCExpr::VariantKind VK = RISCVMCExpr::VK_RISCV_None;
    if (!isImm())
      return false;
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    return IsConstantImm && Imm != 0 && isUInt<5>(Imm) &&
           VK == RISCVMCExpr::VK_RISCV_None;
  }
  ...
  bool isScale() const {
    ...
    int64_t Imm;
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    return IsConstantImm && Imm != 0 && !(Imm & (Imm - 1)) &&
           VK == RISCVMCExpr::VK_RISCV_None;
  }
  ...
  bool isUImm2Lsb0() const {
    ...
    if (!isImm())
      return false;
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    return IsConstantImm && isShiftedUInt<1,1>(Imm) && VK == RISCVMCExpr::VK_RISCV_None;
  }
  ...
  bool isUImm8tbljalm() const {
    ...
    if (!isUImm8())
      return false;
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    return IsConstantImm && isUInt<8>(Imm) && (uint64_t)Imm < UINT64_C(8) &&
           VK == RISCVMCExpr::VK_RISCV_None;
  }
  ...
  bool isUImm8tblj() const {
    ...
    if (!isUImm8())
      return false;
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    return IsConstantImm && isUInt<8>(Imm) && (uint64_t)Imm <= UINT64_C(55) &&
           VK == RISCVMCExpr::VK_RISCV_None;
  }
  ...
  bool isUImm8tbljal() const {
    ...
    if (!isUImm8())
      return false;
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    return IsConstantImm && isUInt<8>(Imm) && (uint64_t)Imm <= UINT64_C(191) &&
           VK == RISCVMCExpr::VK_RISCV_None;
  }
  ...
  bool isSImm13Lsb0Zce() const {
    if (!isImm())
      return false;
    int64_t Imm;
    RISCVMCExpr::VariantKind VK = RISCVMCExpr::VK_RISCV_None;
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    return IsConstantImm && isShiftedInt<12, 1>(Imm) &&
           VK == RISCVMCExpr::VK_RISCV_None;
  }
  ...
  static std::unique_ptr<RISCVOperand> createRetval(unsigned RetvalEncode,
                                                    SMLoc S, bool IsRV64) {
...
bool RISCVAsmParser::MatchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
  ...
  case Match_InvalidSImm17Lsb000:
    return generateImmOutOfRangeError(
        ...
        "operand must be a symbol with %lo modifier or an "
        "multiple of 8 bytes integer in the range");
...
bool RISCVAsmParser::processInstruction(MCInst &Inst, SMLoc IDLoc,
  ...
  case RISCV::C_PUSH_E: {
    ...
    if ((RISCVZCE::RLIST2ENCODE)rlist2 >= RISCVZCE::RLIST2ENCODE::RA) {
      Opc = RISCV::C_PUSH;
      rlist2 = (int64_t)RISCVZCE::convertRlist2ToRlist3(
          (RISCVZCE::RLIST2ENCODE)rlist2);
    } else {
      Opc = RISCV::C_PUSH_E;
    }
    emitToStreamer(Out, MCInstBuilder(Opc)
