...
bool RISCVZceInstOpt::optimiseZceeInstruction(
  ...
  case RISCV::ZEXTH_RV64:
    if (STI->hasStdExtZbb()) {
      ...
      if (SourceReg == DestReg &&
        ...
        BuildMI(MBB, MBBI, DL, TII->get(RISCV::C_ZEXT_H), DestReg)
            .addReg(SourceReg);
  ...
  case RISCV::SEXTB:
    if (STI->hasStdExtZbb()) {
      ...
      if (SourceReg == DestReg &&
          (DestReg >= RISCV::X8 && DestReg <= RISCV::X15)) {
        BuildMI(MBB, MBBI, DL, TII->get(RISCV::C_SEXT_B), DestReg)
            .addReg(SourceReg);
        MBBI->removeFromBundle();
        Modified = true;
      }
  ...
  case RISCV::SEXTH:
    if (STI->hasStdExtZbb()) {
      ...
      if (SourceReg == DestReg &&
          (DestReg >= RISCV::X8 && DestReg <= RISCV::X15)) {
        BuildMI(MBB, MBBI, DL, TII->get(RISCV::C_SEXT_H), DestReg)
            .addReg(SourceReg);
        MBBI->removeFromBundle();
        Modified = true;
      }
  ...
  case RISCV::ANDI: {
    ...
    if (SourceReg1 == DestReg &&
      ...
      BuildMI(MBB, MBBI, DL, TII->get(RISCV::C_ZEXT_B), DestReg)
          .addReg(SourceReg1);
      MBBI->removeFromBundle();
      Modified = true;
    }
