...
llvm::DILocalVariable *CGDebugInfo::EmitDeclare(const VarDecl *VD,
  ...
  if (!VD->hasAttr<BlocksAttr>() && isa<TypedefType>(VD->getType())) {
    const VectorType *Vt = cast<VectorType>(cast<TypedefType>(VD->getType())
      ->getDecl()->getUnderlyingType());
    if (Vt != NULL && Vt->getVectorKind() == VectorType::RISCVVector)
      Expr.push_back(llvm::dwarf::DW_OP_deref);
  }
