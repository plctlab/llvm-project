...
#  llvm-mc -filetype=obj -triple=riscv32 -mattr=+relax -mattr=+experimental-zce %s -o %t.rv32.o
...
#  llvm-objdump -d -M no-aliases --mattr=+experimental-zce --no-show-raw-insn %t.rv32 | FileCheck --check-prefix=TBLJAL %s
# RUN: llvm-objdump -d -M no-aliases --mattr=+experimental-zce --no-show-raw-insn %t.rv64 | FileCheck --check-prefix=TBLJAL %s
# TBLJAL:      tbljalm 0
...
#  llvm-objdump -d -M no-aliases --mattr=+experimental-zce --no-show-raw-insn %t-boundary.rv32 | FileCheck --check-prefix=BOUNDARY %s
# RUN: llvm-objdump -d -M no-aliases --mattr=+experimental-zce --no-show-raw-insn %t-boundary.rv64 | FileCheck --check-prefix=BOUNDARY %s
