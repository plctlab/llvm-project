...
struct CheckToCompatibleSimd {
  ...
  void operator()() {
    static_assert(std::is_same_v<decltype(ex::to_compatible<_Tp, _Np>(ex::fixed_size_simd<_Tp, _Np>())), ex::simd<_Tp>>,
                  "");
...
struct CheckToCompatibleSimdMask {
  ...
  void operator()() {
...

    {
      ...
      auto after_cast = ex::to_compatible<_Tp, _Np>(origin);
      auto expected = ex::simd_mask<_Tp>();
